...
import { Plus, Search, MoreHorizontal, Edit, Trash2, ArrowUpDown, Loader2 } from "lucide-react"
...
export default function AdminMateriales() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (editingMaterial) {
        ...
        toast({ title: "Material actualizado", description: "Actualizado correctamente" })
      } else {
        setMaterials([...materials, data.material])
        toast({ title: "Material creado", description: "Creado correctamente" })
      }
      ...
      toast({
        title: "Error",
        description: error instanceof Error ? error.message : "Error al guardar material",
        variant: "destructive",
      })
    } finally {
  ...
  const handleDelete = async (id: number) => {
    ...
    try {
      ...
      toast({ title: "Material eliminado", description: "Eliminado correctamente" })
    } catch (error) {
      console.error("Error al eliminar material:", error)
      toast({
        title: "Error",
        description: error instanceof Error ? error.message : "Error al eliminar material",
        variant: "destructive",
      })
    }
  }
          ...
          <form onSubmit={handleSubmit}>
            <div className="grid gap-4 py-4">
              ...
              <div className="grid grid-cols-4 items-center gap-4">
                ...
                <Input id="stock" name="stock" type="number" min="0" value={formData.stock} onChange={handleChange} className="col-span-3" />
              </div>
              <div className="grid grid-cols-4 items-center gap-4">
                <Label htmlFor="unit" className="text-right">Unidad</Label>
                <Input id="unit" name="unit" value={formData.unit} onChange={handleChange} className="col-span-3" placeholder="metros, unidades, etc." />
              </div>
            </div>
            <DialogFooter>
