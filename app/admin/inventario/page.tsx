...
export default function AdminInventario() {
  ...
  const handleAdjustStock = async (material: Material) => {
    ...
    try {
      ...
      if (!response.ok)
        throw new Error("Error al cargar detalles del material");
  ...
  const handleAdjustmentChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (adjustmentType === "add") {
      setNewStock(
        editingMaterial?.stock ? editingMaterial.stock + value : value
      );
    } else {
      setNewStock(
        editingMaterial?.stock ? Math.max(0, editingMaterial.stock - value) : 0
      );
    }
  };
  ...
  const handleAdjustmentTypeChange = (type: "add" | "subtract") => {
    ...
    if (type === "add") {
      setNewStock(
        editingMaterial?.stock
          ? editingMaterial.stock + stockAdjustment
          : stockAdjustment
      );
    } else {
      setNewStock(
        editingMaterial?.stock
          ? Math.max(0, editingMaterial.stock - stockAdjustment)
          : 0
      );
    }
  };
                    ...
                    <ul className="space-y-1">
                      {materialUsage.map((usage) => (
                        <li key={usage.productId} className="text-sm">
                          {usage.product.name} - {usage.quantity}{" "}
                          {editingMaterial?.unit || "unidades"} por producto
                        </li>
                      ))}
                    </ul>
                  </div>
